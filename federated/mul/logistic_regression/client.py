...
class SimpleClient(fl.client.NumPyClient):
    ...
    def evaluate(self, parameters, config):
        ...
        loss = log_loss(self.y_test, probabilities, labels=[0,1,2,3,4,5,6,7,8])
...
def create_client(cid: str):
    ...
    X_train,  y_train = utils.split_data(train_partitions[int(cid)-1])
    X_test, y_test = utils.split_data(test_partitions[int(cid)-1])
